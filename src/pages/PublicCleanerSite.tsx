...
export default function PublicCleanerSite({ forcedSlug, customDomain = false }: Props = {}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: cleanerRow, error: cleanerError } = await supabase
        .from("cleaners")
        .select("id, slug, business_name, logo_url, address, phone, whatsapp, website, about, contact_email")
        .eq("slug", slug)
      ...
      const siteData = siteRow as CleanerSite;
      setSite(siteData);
  ...
  useEffect(() => {
    ...
    const businessName = cleaner.business_name || "Local cleaner";
    const description =
      site.description || cleaner.about || `${businessName} provides professional local cleaning services.`;
    const canonicalUrl = customDomain
      ? `${window.location.origin}${window.location.pathname || "/"}`
      : `${window.location.origin}/cleaner/${cleaner.slug}`;

    document.title = `${businessName} | Local Cleaning Services`;
    ...
    if (site.hero_url || cleaner.logo_url) {
      upsertMeta('meta[property="og:image"]', { property: "og:image" }, site.hero_url || cleaner.logo_url || "");
    }
    ...
    structuredData.dataset.cleanerSiteSeo = "true";
    structuredData.text = JSON.stringify({
      ...
      url: canonicalUrl,
      description,
      telephone: cleaner.phone || cleaner.whatsapp || undefined,
      email: cleaner.contact_email || undefined,
      image: site.hero_url || cleaner.logo_url || undefined,
      address: cleaner.address || undefined,
      makesOffer: services.map((service) => ({
        "@type": "Offer",
        name: formatServiceName(service.service, service.variant),
        priceCurrency: "GBP",
        price: service.price_cents == null ? undefined : (service.price_cents / 100).toFixed(2),
      })),
    });
  ...
  }, [cleaner, site, services, customDomain]);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <header className="bg-white border-b border-slate-200">
        <div className="mx-auto max-w-6xl px-5 py-5 flex items-center justify-between gap-4">
          ...
          {!customDomain && <span className="text-xs sm:text-sm text-slate-500">Powered by Klean.ly</span>}
        </div>
      </header>
      ...
      <main>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  {whatsappNumber && (
                    <a
                      ...
                      style={{ backgroundColor: site.accent_color, color: site.primary_color }}
                    >
                      Book on WhatsApp
                    </a>
                  )}
        ...
        {site.show_services && (
          <section className="mx-auto max-w-6xl px-5 py-14 sm:py-20">
            <div className="mb-8">
              <p className="text-sm font-semibold uppercase tracking-wider" style={{ color: site.primary_color }}>
                Services
              </p>
                ...
                {services.map((service) => {
                  ...
                  return (
                    <div key={service.id} className="rounded-3xl bg-white border border-slate-200 p-6 shadow-sm">
                      ...
                      <div className="mt-4 text-2xl font-black">{servicePrice || "Ask for price"}</div>
      ...
      <footer className="text-white/60" style={{ backgroundColor: site.primary_color }}>
        <div className="mx-auto max-w-6xl px-5 py-8 flex flex-col sm:flex-row gap-2 sm:items-center sm:justify-between text-sm">
          ...
          {!customDomain && <Link to="/" className="hover:text-white">Powered by Klean.ly</Link>}
        </div>
      </footer>
    </div>
  );
