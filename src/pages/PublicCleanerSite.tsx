...
export default function PublicCleanerSite({ forcedSlug, customDomain = false }: Props = {}) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-950">
    ...
    <main>
      ...
      {site.show_services && <section className="mx-auto max-w-6xl px-5 py-14 sm:py-20"><p className="text-sm font-semibold uppercase tracking-wider" style={{ color: site.primary_color }}>Services</p><h2 className="text-3xl font-black mt-2 mb-8">What we can help with</h2>{services.length ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{services.map((service) => <div key={service.id} className="rounded-3xl bg-white border p-6 shadow-sm flex flex-col"><div className="text-lg font-bold">{serviceName(service.service, service.variant)}</div><div className="mt-4 text-2xl font-black">{price(service.price_cents) || "Price confirmed by cleaner"}</div><div className="mt-2 text-sm text-slate-500">Choose this service and send your booking details.</div><button type="button" onClick={() => openBooking(service.id)} className="mt-5 rounded-2xl px-5 py-3 font-bold" style={{ backgroundColor: site.primary_color, color: "white" }}>Book this clean</button></div>)}</div> : <div className="rounded-3xl bg-white border p-7 text-slate-600">Services and pricing will appear here once published.</div>}</section>}
      ...
      {site.show_about && <section className="bg-white"><div className="mx-auto max-w-6xl px-5 py-14 sm:py-20 grid gap-8 lg:grid-cols-2"><div><p className="text-sm font-semibold uppercase tracking-wider" style={{ color: site.primary_color }}>About</p><h2 className="text-3xl font-black mt-2">A local business you can contact directly</h2><p className="mt-5 text-slate-600 leading-7">{intro}</p></div>{site.show_contact && <div className="rounded-3xl bg-slate-50 border p-7 space-y-4">{services.length > 0 && <button type="button" onClick={() => openBooking()} className="w-full rounded-2xl px-5 py-3 font-bold text-white" style={{ backgroundColor: site.primary_color }}>Book a clean</button>}{cleaner.phone && <a onClick={() => recordEventFetch({ cleanerId: cleaner.id, event: "click_phone", meta: { source: "cleaner_site" } }).catch(() => undefined)} href={`tel:${cleaner.phone}`} className="block"><div className="text-xs uppercase text-slate-500">Phone</div><div className="font-bold mt-1">{cleaner.phone}</div></a>}{cleaner.contact_email && <a href={`mailto:${cleaner.contact_email}`} className="block"><div className="text-xs uppercase text-slate-500">Email</div><div className="font-bold mt-1 break-all">{cleaner.contact_email}</div></a>}{cleaner.website && <a onClick={() => recordEventFetch({ cleanerId: cleaner.id, event: "click_website", meta: { source: "cleaner_site" } }).catch(() => undefined)} href={websiteUrl(cleaner.website)} target="_blank" rel="noreferrer" className="block"><div className="text-xs uppercase text-slate-500">Website</div><div className="font-bold mt-1 break-all">{cleaner.website}</div></a>}</div>}</div></section>}
    </main>
    ...
    <CleanerBookingModal open={bookingOpen} onClose={() => setBookingOpen(false)} cleanerId={cleaner.id} businessName={cleaner.business_name || "Your cleaner"} services={services} primaryColor={site.primary_color} accentColor={site.accent_color} initialServiceId={bookingServiceId} />
  </div>;
}
